...
Microsoft::WRL::ComPtr<ID3D12Resource> CreateBufferResource(const Microsoft::WRL::ComPtr<ID3D12Device>& device, size_t sizeInBytess)
{
	//頂点リソース用のヒープの設定
...
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> CreateDescriptorHeap(const Microsoft::WRL::ComPtr<ID3D12Device>& device, D3D12_DESCRIPTOR_HEAP_TYPE heapType, UINT numDesctiptors, bool shaderVisible)
{
	//ディスクリプタヒープの生成
...
#pragma region Sprite
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> transformationMatrixResourcesSprite = CreateBufferResource(device, sizeof(TransformationMatrix));
	//データを書き込む
...
#pragma region 球
	...
	//Sphere用の頂点リソースを作る。
	Microsoft::WRL::ComPtr<ID3D12Resource> vertexResourceSphere = CreateBufferResource(device, sizeof(VertexData) * modelData.vertices.size());
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> transformationResourcesSphere = CreateBufferResource(device, sizeof(TransformationMatrix));
	//データを書き込む
	...
	//インデックス用リソース
	//Microsoft::WRL::ComPtr<ID3D12Resource> indexResourceSphere = CreateBufferResource(device, sizeof(uint32_t) * sphereIndexNum);
