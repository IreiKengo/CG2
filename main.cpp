...
ID3D12Resource* CreateBuffrResource(ID3D12Device* device, size_t sizeInBytess)
{
	//頂点リソース用のヒープの設定
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	rtvDescriptorHeapDesc.NumDescriptors = 2;//ダブルバッファ用に２つ。多くても別にかまわない
	...
	//頂点リソースを作る
	ID3D12Resource* vertexResource = CreateBuffrResource(device, sizeof(Vector4) * 3);
	
	//マテリアル用のリソースを作る。今回はカラー１つ分のサイズを用意する
	ID3D12Resource* materialResource = CreateBuffrResource(device, sizeof(Vector4));
	//マテリアルにデータを書き込む
