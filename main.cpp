...
ID3D12Resource* CreateBufferResource(ID3D12Device* device, size_t sizeInBytess)
{
	//頂点リソース用のヒープの設定
...
ID3D12DescriptorHeap* CreateDescriptorHeap(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE heapType, UINT numDesctiptors, bool shaderVisible)
{
	//ディスクリプタヒープの生成
...
#pragma region Sprite
	...
	ID3D12Resource* transformationMatrixResourcesSprite = CreateBufferResource(device, sizeof(TransformationMatrix));
	//データを書き込む
...
#pragma region 球
	...
	ID3D12Resource* transformationResourcesSphere = CreateBufferResource(device, sizeof(TransformationMatrix));
	//データを書き込む
	...
	//インデックス用リソース
	ID3D12Resource* indexResourceSphere = CreateBufferResource(device, sizeof(uint32_t) * sphereIndexNum);
...
#pragma endregion
	...
	transformationResourcesSphere->Release();

	vertexResourceSprite->Release();
	transformationMatrixResourcesSprite->Release();
